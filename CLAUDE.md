...
- **Synchronizace (přepracováno v Buildu 52 — dřív se záznamy z tabletu na PC vůbec neobjevily):**
  ...
  - **`pullFromCloud({light})` čte po stránkách** (`PULL_PAGE = 25`, `order('time',desc).order('id',desc)`, `range()`): nejnovější první, takže dnešní záznamy dorazí hned. Když stránka selže, **co je stažené zůstane** (`incomplete` se jen ohlásí ve stavu) a zbytek se dotáhne příště. Dřív to byl jeden `select('*')` nad celou tabulkou s limitem 20 s → při větší DB spadl a nestáhlo se **nic**; navíc bez `order` hrozil limit 1000 řádků PostgRESTu.
  - **Lehký režim** (auto-sync na pozadí) stahuje jen `LIGHT_COLS` (bez `photo`/`after_photo`/`office`) a plný řádek dotáhne jen u záznamů, které lokálně chybí nebo se liší (`lightRowDiffers`). **Nutné kvůli egress na free tieru** — plný pull tahá všechny base64 fotky (~0,5 MB/záznam), každých 5 minut by to kvótu 5 GB/měsíc spálilo za odpoledne.
  - **Známé omezení lehkého režimu:** změnu *uvnitř* `office`, která nepřeklopí `office_done` ani malá pole, nepozná → dotáhne se až plným syncem (start appky / tlačítko).
