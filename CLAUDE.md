...
## Aktuální stav (v3.2.0-test, Build 55)
...
- **Synchronizace (přepracováno v Buildu 52 — dřív se záznamy z tabletu na PC vůbec neobjevily):**
  ...
  - **Stahování (přepsané v Buildu 55): seznam zvlášť, plné řádky po jednom.** `pullFromCloud({light})` nejdřív čte **jen malá pole** (`LIGHT_COLS`, bez `photo`/`after_photo`/`office`) po stránkách `PULL_PAGE = 50` (`order('time',desc).order('id',desc)` + `range()`, nejnovější první, ~200 B/řádek) a pak stahuje **plný řádek po jednom** přes `eq('id').maybeSingle()` s limitem `PULL_ROW_TIMEOUT` (30 s) — a jen u záznamů, které `needsFullRow()` označí. Do Buildu 54 tahal plný sync `select('*')` po 25 řádcích, tedy i 30 MB v jednom dotazu s limitem 20 s → na pomalejší lince spadl a **nestáhlo se vůbec nic** (hlásilo `✓ hotovo (↑0 ↓0) · ⚠ stahování nedokončeno`).
