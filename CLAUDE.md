...
- **Rychlé přepínání záznamů (Build 53):** dřív se při každém kliknutí v seznamu rušil a znovu stavěl Leaflet (mapa blikala) a `getEntryById` četlo přes `getAllEntries()` → z IndexedDB se načetly **všechny** záznamy včetně base64 fotek (u stovky záznamů desítky MB = sekání). Teď: `getEntryById` čte **klíčem** (`store.get`, ve stejné frontě `idbReadChain`); pravý panel se překresluje po částech (`renderSidePane` → `#es-gps` + `#es-docs`, skeleton z `buildEntrySideHtml` jen poprvé); **mapa se staví jen jednou** (`syncSideMap`) a při přepnutí se přebarví jen předchozí a nový bod (`styleSideMarker`, `znSideMarkerById`, `znSideActiveId`), body se přestaví až při změně `sideMapSig()` (jiná složka/filtr). Naměřeno headless na 100 záznamech s ~300 kB fotkou: **68 → 5 ms** na přepnutí, postavených map na 12 přepnutí **13 → 1**.
...
- **Úprava údajů z výjezdu (Build 50):** dialog **„✏️ Upravit údaje z výjezdu"** (`#edit-entry`, funkce `openEditEntry`/`saveEditEntry`/`refreshAfterEntryEdit`) — mění rovinatost (stejné chipy a živý verdikt jako výjezd, navíc „🗑 Vše"), jména, vyjetou kolej, poznámku, datum a čas (volitelně přesune i do složky podle data) a fotku při kontrole. Otevírá se ze **tří míst**: nabídka ⋯, tlačítko ✏️ v hlavičce detailu, a v Office karta „Měření z výjezdu". Ukládá `synced:false` + hned `pushEntry`, přepočítá `office.rovinatost_verdikt` a překreslí seznam / detail (`viewerRepaint`); v Office **mění jen kartu měření a odznáček verdiktu**, ať se nezahodí rozepsaný formulář. Zavření z kódu ubírá krok v historii přes `dismissEditEntry` + `eePopIgnore` (jinak by `handlePop` zavřel i modál pod ním).
...
- **Synchronizace (přepracováno v Buildu 52 — dřív se záznamy z tabletu na PC vůbec neobjevily):**
  - **Kdy se spouští:** start appky, tlačítko **⟳ Synchronizovat** (nově i v tmavé liště, adresuje se třídou `.sync-btn`), událost `online`, **auto-sync při vrácení do okna** (nejvýš 1×/min) a **časovač každých 5 min**. Do Buildu 51 běžel sync jen při startu a z tlačítka v šuplíku, který je na ≥1000 px nedosažitelný (☰ je skryté) → na PC nešlo sync vůbec vyvolat.
  - **Stahování (přepsané v Buildu 55): seznam zvlášť, plné řádky po jednom.** `pullFromCloud({light})` nejdřív čte **jen malá pole** (`LIGHT_COLS`, bez `photo`/`after_photo`/`office`) po stránkách `PULL_PAGE = 50` (`order('id',desc)` + `range()`, ~200 B/řádek) a pak stahuje **plný řádek po jednom** přes `eq('id').maybeSingle()` s limitem `PULL_ROW_TIMEOUT` (30 s) — a jen u záznamů, které `needsFullRow()` označí. Do Buildu 54 tahal plný sync `select('*')` po 25 řádcích, tedy i 30 MB v jednom dotazu s limitem 20 s → na pomalejší lince spadl a **nestáhlo se vůbec nic** (hlásilo `✓ hotovo (↑0 ↓0) · ⚠ stahování nedokončeno`).
  - **`needsFullRow(existing, r, deep)`:** stáhni plný řádek, když se liší malá pole (`lightRowDiffers`); u **plného** syncu navíc když lokálně **chybí fotka** nebo když cloud má `office_done` a lokálně není `office` (záznam se do zařízení nikdy celý nedostal). Tím pull nikdy nestahuje base64 zbytečně — **nutné kvůli egress na free tieru** (~0,5 MB/záznam, auto-sync každých 5 min by kvótu 5 GB/měsíc spálil za odpoledne).
  - **Jeden nepovedený záznam nezabije sync:** chyba jednoho řádku se počítá do `failed` (ohlásí se „⚠ N záznamů se nestáhlo"), ostatní projdou a příště se to zkusí znovu. `maybeSingle` (ne `single`) znamená, že mezitím smazaný řádek **není chyba** — `single` na nulu řádků vrací „cannot coerce the result to a single JSON object". Když spadne stránka seznamu, co je stažené zůstane a zbytek se dotáhne příště.
  - **Známé omezení (platí i pro plný sync):** změnu *uvnitř* `office`, která nepřeklopí `office_done` ani malá pole, sync nepozná. Záchranná brzda je nabídka ⋯ → **„⤓ Stáhnout znovu z cloudu"** (`refetchEntryFromCloud`) = přepsání jednoho záznamu verzí z cloudu (u `synced:false` se ptá na potvrzení).
  - **Řadí se podle `id`, ne podle `time` (Build 56).** Na `time` **není index** (tabulka má jen PK na `id`), takže `order('time')` nutil Postgres přečíst a seřadit celou tabulku — a dokud se řadilo se `select('*')`, protahoval sort i base64 fotky. Server dotaz zabil vlastním limitem: **`canceling statement due to statement timeout`** (skutečná chyba od uživatele, Build 52–55). `order('id',desc)` jede po primárním klíči (index scan, žádné řazení) a protože `id` je `String(Date.now())`, je to zároveň nejnovější první. **Kdyby bylo potřeba řadit podle `time`, musí nejdřív vzniknout index `(time desc)`** — to je změna schématu, takže se na ni ptát (viz „Plánované práce" bod 2).
  - **Hlášení jde klepnout:** `.sync-state` má `title` s celým textem a na klik `showSyncDetail()` ukáže stav, **poslední chybu** (`lastSyncError`) a počet čekajících — v liště se text zkracuje na 240 px, takže chyba byla dřív nečitelná.
